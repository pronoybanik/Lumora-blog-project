      ...
      >
        ...
        {blog.coverImage ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-indigo-700">
            <Compass className="w-10 h-10 text-white/90" strokeWidth={1.5} />
          </div>
        )}
      </div>
...
function EditProfileModal({ profile, onClose, onSave, saving }) {
  ...
  const handleAvatarUpload = async (event) => {
    ...
    setAvatarMessage("Uploading image...");

    try {
      const uploadData = new FormData();
      uploadData.append("file", file);
      uploadData.append("upload_preset", CLOUDINARY_UPLOAD_PRESET);
      uploadData.append("folder", "lumora/avatars");
      ...
      setAvatarMessage(error?.message || "Failed to upload image.");
    } finally {
                ...
                {field.name === "avatar" ? (
                  <div className="space-y-3">
                    ...
                    <input
                      ...
                      className="block w-full text-sm text-slate-500 file:mr-3 file:rounded-lg file:border-0 file:bg-indigo-50 file:px-3 file:py-2 file:text-sm file:font-medium file:text-indigo-700 hover:file:bg-indigo-100 disabled:opacity-50"
                    />

                    <p className="text-xs text-slate-500">
                      {avatarMessage || "PNG, JPG, or WEBP up to 5MB."}
                    </p>
                  </div>
                ) : field.textarea ? (
                  <textarea
...
export default function ProfilePage() {
  ...
  const blogStats = useMemo(() => {
    ...
    const totalViews = blogs.reduce(
      (total, blog) => total + Number(blog.viewCount || 0),
      0,
    );
  ...
  const profession = profile.profession || "Lumora User";
    ...
    <div className="bg-slate-50 min-h-screen font-sans">
      ...
      >
        ...
        <div className="relative flex flex-col items-center pt-8 text-center">
          ...
          <p className="text-sm text-slate-600 mt-1">{profession}</p>
          ...
          <nav className="flex gap-8 mt-6 text-sm font-medium text-slate-500">
            ...
            <button className="hover:text-slate-800">Publications</button>

            <button className="hover:text-slate-800">Activity</button>
          ...
          <div className="flex flex-col sm:flex-row sm:items-end gap-5">
            ...
            <div className="pb-2 sm:pb-3">
              ...
              <div className="flex gap-2 mt-3">
                ...
                <SocialLink href={profile.github} icon={Link2} label="GitHub" />
                ...
                {user?.email && (
                  <a
                    ...
                    title="Email"
                    className="min-h-9 flex items-center gap-2 px-3 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-600 transition text-sm"
                  >
